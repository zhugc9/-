...
class APIService:
    ...
    def _select_model(self, stage_key: str, explicit_model: str = None) -> str:
        ...
        # 社交媒体模型：从social_media.models获取
            ...
            social_models = social_config.get('models', {})
            stage_mapping = {
                'VK_BATCH': 'vk_batch_analysis',
                'VK_BATCH_LONG': 'vk_batch_analysis_long',
                'ZHIHU_CHUNKING': 'zhihu_chunking',
                'ZHIHU_ANALYSIS_SHORT': 'zhihu_analysis_short',
                'ZHIHU_ANALYSIS': 'zhihu_analysis_standard',
                'ZHIHU_ANALYSIS_LONG': 'zhihu_analysis_long'
            }
            model_key = stage_mapping.get(stage_key)
            if model_key:
                primary = social_models.get(model_key)
    ...
    async def _call_api(self, prompt, language='zh', model_name=None, stage_key=None, context_label=None):
        ...
        # 从新配置结构读取语言配置
        general_config = self.config.get('processing', {}).get('general', {})
        language_configs = general_config.get('language_configs', {})
        ...
        if prompt_tokens < threshold_short:
            timeout = timeout_config.get('timeout_short_sec', 400)  # 短文本
        elif prompt_tokens <= threshold_medium:
            timeout = timeout_config.get('timeout_medium_sec', 500)  # 中等文本
        else:
            timeout = timeout_config.get('timeout_long_sec', 500)  # 长文本
...
class SyncAPIService:
    ...
    def _handle_failure(self, stage_key: str) -> bool:
        ...
        if model_pools and 'primary_models' in model_pools:
            primary = model_pools['primary_models'].get(stage_key)
            fallback_list = model_pools.get('fallback_models', {}).get(stage_key, [])
            fallback = fallback_list[0] if fallback_list and fallback_list[0] else None
    ...
    def call_api_sync(self, prompt, language='zh', stage_key=None):
        ...
        lang_config = language_configs.get(language, language_configs.get('zh', {}))
        ...
        for attempt in range(attempts_per_model):
            try:
                ...
                timeout = lang_config.get('timeout', 180)
