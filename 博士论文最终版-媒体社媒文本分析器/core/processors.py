...
class VKProcessor(BaseProcessor):
    ...
    def process(self, df, output_path, source='vk'):
        ...
        config = language_configs.get('ru', {
            'timeout': 180,
            'max_concurrent': 2,
    ...
    def _process_batch(self, batch_task):
        ...
        try:
            ...
            # 获取VK长文本阈值
            social_config = self.config.get('social_media', {})
            text_thresholds = social_config.get('text_length_thresholds', {})
            VK_LONG_TEXT_THRESHOLD = text_thresholds.get('vk_long_text', 1500)
            if not VK_LONG_TEXT_THRESHOLD:
                # 向后兼容
                VK_LONG_TEXT_THRESHOLD = self.config.get('VK_LONG_TEXT_THRESHOLD', 1500)
