...
class ReliabilityTestModule:
    ...
    def locate_unit(self, unit_text: str, full_text: str) -> Optional[Tuple[int, int]]:
        """
        改进版文本定位算法
        修复了模糊匹配边界不精确的问题，并简化了标准化匹配逻辑
        """
        if not unit_text or not full_text:
            return None
            
        # 第一级：精确匹配
        start_idx = full_text.find(unit_text)
        if start_idx != -1:
            return (start_idx, start_idx + len(unit_text))
        
        # 第二级：高效模糊匹配（使用RapidFuzz内置搜索，废除手动滑动窗口）
        similarity_threshold = 88  # 适应字母语言的阈值
        
        # 【SOTA优化】在匹配前统一转为小写
        unit_text_lower = unit_text.lower()
        full_text_lower = full_text.lower()
        
        # 【核心优化】直接使用process.extractOne在全文中搜索，废除手动循环
        result = process.extractOne(
            unit_text_lower, 
            [full_text_lower],  # 将全文作为搜索目标
            scorer=fuzz.partial_ratio, 
            score_cutoff=similarity_threshold
        )
        
        if result:
            # result 是 (matched_text, score, choice_index)
            matched_text_lower = result[0]
            score = result[1]
            
            # 在原始大小写的full_text中找到对应位置
            # 使用更精确的查找方法
            start_index = full_text_lower.find(matched_text_lower)
            if start_index != -1:
                end_index = start_index + len(matched_text_lower)
                return (start_index, end_index)
            else:
                # 备用方案：使用更宽松的查找
                return self._fuzzy_locate_in_window(
                    unit_text, full_text, 0
                )
        
        return None
    
    def _fuzzy_locate_in_window(self, unit_text: str, window_text: str, window_start: int) -> Optional[Tuple[int, int]]:
        """
        在窗口内进行精确的模糊定位（备用方法）
        使用逐字符比较寻找最佳匹配子串
        """
        ...
        # 在窗口内滑动unit_text长度的子串
        for i in range(len(window_text) - unit_len + 1):
            sub_text = window_text[i:i + unit_len]
            score = fuzz.ratio(unit_text, sub_text)
            if score > best_score:
                best_score = score
                best_position = (window_start + i, window_start + i + unit_len)
    ...
    def generate_positive_test_file(self, df_all_results: pd.DataFrame, df_all_input: pd.DataFrame) -> None:
        ...
        for source, config in self.sampling_config.items():
            ...
            sampled = source_results.sample(n=sample_size, random_state=42)
    ...
    def generate_negative_test_file(self, df_all_results: pd.DataFrame, df_all_input: pd.DataFrame) -> None:
        ...
        for source, config in self.sampling_config.items():
            ...
            sampled = source_input.sample(n=sample_size, random_state=42)
    ...
    def generate_reliability_files(self) -> None:
        ...
        for input_file in input_files:
            ...
            try:
                ...
                if self.id_column not in df_input_single.columns or self.text_column not in df_input_single.columns:
    ...
    def _generate_negative_samples(self, df_analyzed: pd.DataFrame, df_input: pd.DataFrame) -> list:
        ...
        sample_size = min(recall_count, len(source_input))
        sampled_articles = source_input.sample(n=sample_size, random_state=42)
...
def create_reliability_test_module(input_path: str, output_path: str, sampling_config: dict,
                                 id_column: str = "序号", text_column: str = "text") -> ReliabilityTestModule:
    return ReliabilityTestModule(input_path, output_path, sampling_config, id_column, text_column)
