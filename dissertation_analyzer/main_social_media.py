            ...
            try:
                ...
                if '知乎' in sources_in_data:
                    ...
                    if not zhihu_data.empty and zhihu_id_col in zhihu_data.columns:
                        ...
                        # 复制原始知乎输入到模块期望的临时目录，便于列匹配
                        try:
                            temp_input_dir = os.path.join(OUTPUT_PATH, '_reliability_inputs')
                            os.makedirs(temp_input_dir, exist_ok=True)
                            source_input_file = os.path.join(INPUT_PATH, f'{os.path.basename(zhihu_temp_path).replace("(不能删)analyzed_", "")}')
                            if os.path.exists(source_input_file):
                                shutil.copy2(source_input_file, temp_input_dir)
                        except Exception as copy_err:
                            UX.warn(f"复制知乎原始输入失败: {copy_err}")
